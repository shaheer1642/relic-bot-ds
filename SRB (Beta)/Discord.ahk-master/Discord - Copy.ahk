class Discord
{
	...
	CallAPI(Method, Endpoint, Data="")
	{
		...
		if (Http.status != 200 && Http.status != 204)
		{
			...
			Fileappend, % "[" A_DD "/" A_MMM "/" A_YYYY " " A_Hour ":" A_Min "] Error occured while calling API in script " A_ScriptName " at line " A_LineNumber "`nResponse status: " Http.status "`nServer Response: " Http.ResponseText "`r`n", Error log.dat
	...
	OP0_MESSAGE_CREATE(Data)
	{
		...
		*/
		...
		if (messageContent[1]==(this.prefix . "help"))
		{
			...
			d_postdata.embeds[1].color := "5225082"
			;-----------------------------
			d_postdata.embeds[1].fields.push({name: ".uptime", value: "Reports current uptime`nUsage example:`r`n.uptime"}, {name: ".orders <item_name>", value: "Retrieve top 5 sell orders for an item from warframe.market`nUsage example:`n.orders frost prime`n.orders ember`n.orders kronen prime blade`n.orders axi L4 relic`n.orders primed pressure point"}, {name: ".relics <prime_item>", value: "Retrieve relics for a prime item`nUsage example:`r`n.relics frost_prime_blueprint"}, {name: ".auctions <kuva_weapon>", value: "Retrieve auctions for a kuva weapon lich from warframe.market, sorted by buyout price and weapon damage`nUsage example:`r`n.auctions kuva_kohm"}, {name: ".list <prime_item>", value: "List a prime item on warframe.market on your profile as the top selling order (requires authorization)`nUsage example:`n.list frost_prime_blueprint`n.list frost_prime_blueprint +10`n.list frost_prime_blueprint -20"}, {name: ".relist all", value: "Exactly like .list command except it relists all the sell orders on your profile for prime items. (requires authorization)`nIn order to prevent stress on the API, you can only use this command once per hour.`nUsage example:`r`n.relist all`n.relist all +10`n.relist all -20"})
			this.CallAPI("POST", "/channels/" Data.channel_id "/messages", d_postdata)
			this.CallAPI("PUT", "/channels/" Data.channel_id "/messages/" opId "/reactions/✅/@me")
			return
		}
		...
		if (messageContent[1]==(this.prefix . "uptime"))
		{
			...
			this.SendMessage(Data.channel_id, "Current uptime: " MillisecToTime(A_TickCount-uptime))
			this.CallAPI("PUT", "/channels/" Data.channel_id "/messages/" opId "/reactions/✅/@me")
			return
		}

		/*
		if (messageContent[1]==(this.prefix . "orders"))
		{
			if (messageContent[2]=="")
			{
				this.SendMessage(Data.d.channel_id, "Usage example:`r`n.orders frost_prime_blueprint")
				this.CallAPI("PUT", "/channels/" Data.d.channel_id "/messages/" opId "/reactions/✅/@me")
				return
			}
			if !(messageContent[3]=="")
			{
				this.SendMessage(Data.d.channel_id, "Incorrect command. Usage example:`r`n.orders frost_prime_blueprint")
				return
			}
			messageId := (this.SendMessage(Data.d.channel_id, "Processing")).id
			d_item_url := messageContent[2]
			try 
			{
				FileDelete, orderCache.json
			}
			catch e 
			{
				if !(e.message=="1")
				{
					this.CallAPI("PATCH", "/channels/" Data.d.channel_id "/messages/" messageId, {"content": "Unexpected file delete error. Contant MrSofty#7926"})
					return
				}
			}
			d_status := srbFunctions.getOrder(d_item_url,"orderCache.json")		;func from main code
			if (d_status==-1)
			{
				this.CallAPI("PATCH", "/channels/" Data.d.channel_id "/messages/" messageId, {"content": "Error occured. Please try again"})
				;Discord.SendMessage(Data.d.channel_id, "Error occured. Please try again")
				return
			}
			if (d_status==-2)
			{
				this.CallAPI("PATCH", "/channels/" Data.d.channel_id "/messages/" messageId, {"content": "No sellers found at the moment for " d_item_url})
				this.CallAPI("PUT", "/channels/" Data.d.channel_id "/messages/" opId "/reactions/✅/@me")
			...
			if (d_status==-3)
			{
				this.CallAPI("PATCH", "/channels/" Data.d.channel_id "/messages/" messageId, {"content": "Item " d_item_url " not found"})
				;Discord.SendMessage(Data.d.channel_id, "Item " d_item_url " not found")
				return
			}
			FileRead, d_orderDetail, orderCache.json
			if (ErrorLevel==1)
			{
				this.CallAPI("PATCH", "/channels/" Data.d.channel_id "/messages/" messageId, {"content": "File read error. Please contact MrSofty#7926"})
				return
			}
			d_orderDetail := StrReplace(d_orderDetail, """" . d_item_url . """: ", "")
			d_orderDetail := JSON.Load(d_orderDetail)
			d_orderDetail := objectSort(d_orderDetail, "price",,false)
			d_postdata := "{""content"":""React with :up: to update"",""embeds"": [{ ""fields"": ["
			d_sellerNames := ""
			d_partQuantity := ""
			d_partPrice := ""
			loop, 5		;list already sorted
			{
				d_sellerNames := d_sellerNames . d_orderDetail[A_index].seller . "\n"
				d_partQuantity := d_partQuantity . d_orderDetail[A_index].quantity . "\n"
				d_partPrice := d_partPrice . d_orderDetail[A_index].price . "\n"
			}
			...
			d_postdata := d_postdata . """url"": ""https://warframe.market/items/" d_item_Url """}]}"
			d_postdata := JSON.Load(d_postdata)
			try
			{
				this.CallAPI("PUT", "/channels/" Data.d.channel_id "/messages/" (this.CallAPI("PATCH", "/channels/" Data.d.channel_id "/messages/" messageId, d_postdata)).id "/reactions/🆙/@me")
				this.CallAPI("PUT", "/channels/" Data.d.channel_id "/messages/" opId "/reactions/✅/@me")
				return
			}
			catch e
		...
		*/
		...
		if (messageContent[1]==(this.prefix . "relics"))
		{
			if (messageContent[2]=="")
			{
				this.SendMessage(Data.channel_id, "Usage example:`r`n.relics frost_prime_blueprint")
				this.CallAPI("PUT", "/channels/" Data.channel_id "/messages/" opId "/reactions/✅/@me")
				return
			}
			if !(messageContent[3]=="")
			{
				this.SendMessage(Data.channel_id, "Incorrect command. Usage example:`r`n.relics frost_prime_blueprint")
				return
			}
			messageId := (this.SendMessage(Data.channel_id, "Processing")).id
			d_item_url := messageContent[2]
			file_path := "..\Prime Parts Info\" d_item_url ".json"
			Fileread, pricesDBContent, pricesDB.json
			if (ErrorLevel==1)
			{
				this.CallAPI("PATCH", "/channels/" Data.channel_id "/messages/" messageId, {"content": "File read error. Please contact MrSofty#7926"})
				return
			...
			try
			{
				pricesDBContent := JSON.Load(pricesDBContent)
			}
			catch e
			{
				this.SendMessage(Data.channel_id, "Error occured. Please try again")
				return
			}
			loop % pricesDBContent.MaxIndex()
			{
				if (pricesDBContent[A_index].item_url==d_item_url)
				{
					relicNames := pricesDBContent[A_index].relics
					relicNames := StrReplace(relicNames, "_", " ")
					relicNames := StrReplace(relicNames, "/", "`r`n")
					...
					try
					{
						this.CallAPI("PATCH", "/channels/" Data.channel_id "/messages/" messageId, {"content": relicNames})
						this.CallAPI("PUT", "/channels/" Data.channel_id "/messages/" opId "/reactions/✅/@me")
						return
					}
					catch e
		...
		if (messageContent[1]==(this.prefix . "auctions"))
		{
			if (messageContent[2]=="")
			{
				this.SendMessage(Data.channel_id, "Usage example:`r`n.auctions kuva_kohm")
				this.CallAPI("PUT", "/channels/" Data.channel_id "/messages/" opId "/reactions/✅/@me")
				return
			}
			if !(messageContent[3]=="")
		...
		if (messageContent[1]==(this.prefix . "list"))
		{
			...
			loop 
			{
				try 
				{
					...
					{
						if !(e.message=="1")
							throw Exception("Unhandled file delete Error")
					}
					*/
					orders_file_name := "cache\orderCache" A_Now ".json"
			...
			try 
			{
				...
				loop
				{
					try
					{
						...
						WebRequest.Send()
						WebRequest.WaitForResponse()
						if (WebRequest.Status == 500)
							continue
						if (WebRequest.Status == 502)
							continue
						if (WebRequest.Status == 504)
							continue
						if (WebRequest.Status != 200)
						{
							this.CallAPI("PATCH", "/channels/" Data.channel_id "/messages/" messageId, {"content": "<@253525146923433984> Unhandled error. Check log file"})
				...
				this.CallAPI("PATCH", "/channels/" Data.channel_id "/messages/" messageId, {"content": "All prime items sell orders have successfully been relisted for profile " ingame_name})
				this.CallAPI("PUT", "/channels/" Data.channel_id "/messages/" opId "/reactions/✅/@me")
				break
			}
			catch e 
