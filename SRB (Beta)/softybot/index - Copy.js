...
//relic bot "token": "ODMyNjgyMzY5ODMxMTQxNDE3.YHnV4w.JRy0L2nCzOY4QIAU6ylkjG_UOm4",
...
function trades_update() {
    ...
    fs.readdirSync("../WF Events").forEach(file => {
        ...
        if ((WF_log.trades) && (JSON.stringify(WF_log.trades) != "{}")) {
            WF_log.trades.forEach(trade => {
                if (trade.offeringItems.some(e => /platinum/.test(e))) {
                    trade.receivingItems.forEach(relic => {
                        ...
                        if (relic.match("relic")) {
                            if (!relicsArr[(relic)])
                                relicsArr[relic] = {alltime: {total_plat: 0,quantity: 0,avg: 0}, monthly: {total_plat: 0,quantity: 0,avg: 0}}
                            if (new Date().toISOString().substring(0,7) + filestats.ctime.toISOString().substring(0,7)) {   //push to monthly and alltime
                                ...
                                relicsArr[relic].alltime.avg = Math.round(relicsArr[relic].alltime.total_plat/relicsArr[relic].alltime.quantity)
                                //---
                                ...
                                relicsArr[relic].monthly.avg = Math.round(relicsArr[relic].monthly.total_plat/relicsArr[relic].monthly.quantity)
                            }
    ...
    relicsArrAlltime.forEach(e => {
        if (e.avg > 2) {
            value1 += e.relic.replace(/_relic/g,'').replace(/_/g, " ").replace(/(^\w{1})|(\s+\w{1})/g, letter => letter.toUpperCase()) + '\n'
            value2 += e.quantity + '\n'
            value3 += e.avg + 'p\n'
        }
    })
    ...
    let embed2 = []
    embed2.title = 'Relics bought All Time'
    embed2.fields = [
        {name: 'Relic',value: value1,inline: true},
        {name: 'Quantity',value: value2,inline: true},
        {name: 'Avg Price',value: value3,inline: true}
    ]
    client.channels.cache.get('887946206213464105').send({content: ' ', embeds: [embed1,embed2]})
    console.log(embed1)
    console.log(embed2)
    console.log('done')
}
