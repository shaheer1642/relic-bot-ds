...
;----
RegRead wfLogReader, HKCU, Software\Softy Relic Bot, wfLogReader
if ErrorLevel {
	RegWrite REG_DWORD, HKCU, Software\Softy Relic Bot, wfLogReader, 0		;default setting
    RegRead wfLogReader, HKCU, Software\Softy Relic Bot, wfLogReader
}
if wfLogReader
    Settimer, wfLogRead, 2000
...
RegRead enableRandomizer, HKCU, Software\Softy Relic Bot, enableRandomizer
if ErrorLevel {
	RegWrite REG_DWORD, HKCU, Software\Softy Relic Bot, enableRandomizer, 0		;default setting
    RegRead enableRandomizer, HKCU, Software\Softy Relic Bot, enableRandomizer
}
...
Gui, main_interface:Add, CheckBox, x890 y270 checked%enableRandomizer% genableRandomizer venableRandomizer, Enable Randomizer 
...
WinSet, Redraw, , ahk_id %RelicsListView%
return
...
filterRelics:
    Gui, main_interface:default
    Gui, main_interface:ListView,RelicsListView
    GuiControlGet, filter_text, main_interface:
    if (filter_text=="")
    {
        FileRead, relicsArr, SrbDB.json
        relicsArr := JSON.Load(relicsArr)
...
updateBuy:
    ...
    loop
    {
        if (linkerContent=="")
            break
        startPos := 1
        endPos := 280-25-13        ;last is extra characters for WTB and price
...
enableRandomizer:
    GuiControlGet, enableRandomizer
	RegWrite REG_DWORD, HKCU, Software\Softy Relic Bot, enableRandomizer, %enableRandomizer%
    return
...
ducatPartsUpdate:       ;will be executed every 5 minutes
        ...
        if (totalQuantity>=6 && avgPrice<=10)
        {
            ...
            loop % presence_list.members.MaxIndex()
            {
                if InStr(JSON.Dump(presence_list.members[A_index].roles), "874077155083026473")
                {
                    ...
                    if (user_id == "832682369831141417")    ;skip for bot
                        continue
                    hasFound := 0
    ...
    if (postdata.content == "") || (postdata.content == "[]") || (postdata.content == "{}")
        return
...
wfLogRead:
    log_path := "C:\Users\Horcrux\AppData\Local\Warframe\EE.log"
    
    FileGetSize Size, %log_path%   ; put your filename here
    If (Size0 == Size)
        Return
    ...
    FileRead, logContents, %log_path%
    logContents := SubStr(logContents, 1, StrLen(logContents)-2)
    logContentsArr := StrSplit(logContents, "`r`n")
    logPrefix := ""
    Loop % logContentsArr.MaxIndex()
    {
        if InStr(logContentsArr[A_index], "[Diag]: Current time:")
        {
            logContentsArr[A_index] := StrReplace(logContentsArr[A_index], " ", "")
            logPrefix := StrSplit(logContentsArr[A_index], "UTC:", "]")
            logPrefix := logPrefix[2]
            logPrefix := StrReplace(logPrefix, ":", "")
            break
        }
    }
    if (logPrefix=="")      ; Current time is not written yet
        return
    ; Read log file
    FileRead, log_event, % "WF Events\" logPrefix "_Log.json"
    if ErrorLevel
    {
        file2 := FileOpen("WF Events\" logPrefix "_Log.json", "w")
        file2.write("{}")
        file2.close()
        log_event := {}
        log_event.trades := {}
        log_event.mission_initialize := {}
    }
    else 
        log_event := JSON.Load(log_event)
    ; Log trade events
    start_line := logContentsArr.MaxIndex() - 1000
    if (start_line < 1)
        start_line := 1
    Loop % logContentsArr.MaxIndex()
    {
        ...
        log_seq := ""
        complete_seq := ""
        if InStr(logContentsArr[start_line], "Script [Info]: Dialog.lua: Dialog:`:CreateOkCancel(description=Are you sure you want to accept this trade?")
        {
            log_seq := StrSplit(logContentsArr[start_line], " ")
            log_seq := "s_" log_seq[1]
            loop % log_event.trades.MaxIndex()
            {
                if (log_event.trades[A_index].log_seq==log_seq)        ; Event already handled
                {
                    eventHandled := 1
                    start_line++
                    break
            ...
            if eventHandled
                continue
            i := start_line + 1
            loop, 50    ; Must be in next 50 lines
            {
                if InStr(logContentsArr[i], "Script [Info]: Dialog.lua: Dialog:`:CreateOk(description=The trade was successful!, leftItem=/Menu/Confirm_Item_Ok)")
                {
                    tradeSuccess := 1
                    complete_seq := StrSplit(logContentsArr[i], " ")
                    complete_seq := "s_" complete_seq[1]
                    break
                }
                i++
            }
            ...
            loop 
            {
                logContentsArr[i] := StrReplace(logContentsArr[i], "`r", "")
                logContentsArr[i] := StrReplace(logContentsArr[i], "`n", "")
                if InStr(logContentsArr[i], "leftItem=/Menu/Confirm_Item_Ok, rightItem=/Menu/Confirm_Item_Cancel)")
                {
                    temp := StrSplit(logContentsArr[i], ",")
                    allItems .= temp[1]
                    break
                }
                if (logContentsArr[i] != "") && (logContentsArr[i] != "`r")
                    allItems .= logContentsArr[i] "`r`n"
                i++
            }
            allItems := StrSplit(allItems, "`r`n")
            loop % allItems.MaxIndex()
            {
                if InStr(allItems[A_index], "and will receive from")
                {
                    temp := StrSplit(allItems[A_index], " ")
                    trader := temp[5]
                    i := A_index+1
                    loop
                    {
                        StringLower, item, % allItems[i]
                        receivingItems.push(strreplace(item," ","_"))
                        if (i==allItems.MaxIndex())
                            break
                        i++
                    }
                    break
                }
                StringLower, item, % allItems[A_index]
                offeringItems.push(strreplace(item," ","_"))
            }
            ;msgbox % JSON.Dump(offeringItems) "`n`n" JSON.Dump(receivingItems)
        }
        if tradeSuccess
        {
            log_event.trades.push({log_seq: log_seq, complete_seq: complete_seq, trader: trader, offeringItems: offeringItems, receivingItems: receivingItems, status: "successful"})
        ...
        log_seq := ""
        complete_seq := ""
        if InStr(logContentsArr[start_line], "Script [Info]: Dialog.lua: Dialog:`:CreateOkCancel(description=Are you sure you want to equip")
        {
            ;msgbox % logContentsArr[start_line]
            log_seq := StrSplit(logContentsArr[start_line], " ")
            log_seq := "s_" log_seq[1]
            loop % log_event.mission_initialize.MaxIndex()
            {
                if (log_event.mission_initialize[A_index].log_seq==log_seq)        ; Event already handled
                {
                    eventHandled := 1
                    start_line++
                    break
            ...
            if eventHandled
                continue
            i := start_line + 1
            loop, 20    ; Confirmation must be in next 20 lines
            {
                if InStr(logContentsArr[i], "Script [Info]: Dialog.lua: Dialog:`:SendResult(4)")
                {
                    equipSuccess := 1
            ...
            loop
            {
                if (log_event.mission_initialize[i].log_seq=="")    ; Max index removed
                    break
                if (log_event.mission_initialize.MaxIndex()=="")    ; No data yet
                    break
                if (log_event.mission_initialize[i].complete_seq == "N/A Yet")
                {
                    log_event.mission_initialize.removeat(i)
                    continue
                }
                if (i==log_event.mission_initialize.MaxIndex())
                    break
                i++
            }
            ; Retrieve relic being equipped
            temp := StrSplit(logContentsArr[start_line], " ")
            relicEquipped := temp[12] "_" temp[13] "_" temp[14]
            StringLower, relicEquipped, relicEquipped
            ; Commit event to file
            log_event.mission_initialize.push({log_seq: log_seq, complete_seq: complete_seq, relicEquipped: relicEquipped, status: "unsuccessful"})
        }
        eventHandled := 0
        log_seq := ""
        relicEquipped := ""
        complete_seq := ""
        if InStr(logContentsArr[start_line], "Script [Info]: ProjectionRewardChoice.lua: Got rewards")
        {
            complete_seq := StrSplit(logContentsArr[start_line], " ")
            complete_seq := "s_" complete_seq[1]
            loop % log_event.mission_initialize.MaxIndex()
            {
                if (log_event.mission_initialize[A_index].complete_seq==complete_seq)        ; Event already handled
                {
                    eventHandled := 1
                    start_line++
                    break
            ...
            loop % log_event.mission_initialize.MaxIndex()
            {
                if (log_event.mission_initialize[A_index].complete_seq == "N/A Yet")
                {
                    log_event.mission_initialize[A_index].complete_seq := complete_seq
                    log_event.mission_initialize[A_index].status := "successful"
                    ; Retrieve info for event handle
                    log_seq := log_event.mission_initialize[A_index].log_seq
                    relicEquipped := log_event.mission_initialize[A_index].relicEquipped
                    break
                }
            }
            wfMissionHandler(log_seq,complete_seq,relicEquipped,"successful")
            gosub filterRelics
        }
        start_line++
    }
    file2 := FileOpen("WF Events\" logPrefix "_Log.json", "w")
    file2.write(JSON.Dump(log_event))
    file2.close()
    return
...
$^Numpad8::
    ...
    if (enableRandomizer)
    {
        FileRead, Random_CP, Random_CP.json
        Random_CP := JSON.load(Random_CP)
        low := 1
        high :=  Random_CP.MaxIndex()
        Random, num, low, high
        clipboard := ""
        clipboard := Random_CP[num]
        ClipWait, 2
    }
    else {
        clipboard := ""
...
fetchItemsList()
{
    ...
    try
    {
        loop 
        {
            ...
            if (SubStr(requestItemList.Status, 1, 1) == 5)   ;internal server error
                continue
            break
        }
    }
    ...
    try
    {
        loop 
        {
            if (A_index!=1)
                sleep 500
            requestItemList := ComObjCreate("WinHttp.WinHttpRequest.5.1")
            requestItemList.Open("GET", "https://api.warframe.market/v1/lich/weapons", true)
            requestItemList.Send()
            requestItemList.WaitForResponse()
            if (SubStr(requestItemList.Status, 1, 1) == 5)   ;internal server error
                continue
    ...
    fileList := FileOpen("WFM_Lich_List.json", "w")
    fileList.Write(requestItemList.ResponseText)
    fileList.close()
    return
    ;----------
...
wfTradeHandler(log_seq,complete_seq,trader,offeringItems,receivingItems,status)
{
    FileRead, p_relicListArr, SrbDB.json
    p_relicListArr := JSON.Load(p_relicListArr)
    loop % receivingItems.MaxIndex()
    {
        name := StrReplace(receivingItems[A_index], "_Relic", "")
        name := StrReplace(name, "_", " ")
        StringUpper, name, name, T
        ;msgbox %name% hi
        loop % p_relicListArr.MaxIndex()
        {
            if (p_relicListArr[A_index].name==name)
            {
                p_relicListArr[A_index].quantity++
                break
            }
        }
    }
    file := FileOpen("SrbDB.json", "w")
    file.write(JSON.Dump(p_relicListArr))
    file.close()
    return
}
wfMissionHandler(log_seq,complete_seq,relicEquipped,status)
{
    FileRead, p_relicListArr, SrbDB.json
    p_relicListArr := JSON.Load(p_relicListArr)
    ;----
    name := StrReplace(relicEquipped, "_relic", "")
    name := StrReplace(name, "_", " ")
    StringUpper, name, name, T
    ;msgbox %name% hi
    loop % p_relicListArr.MaxIndex()
...
/*
`::
    Loop Files, %A_ScriptDir%\WF Events\*.json  ; Recurse into subfolders.
    {
        Fileread, logData, %A_ScriptDir%\WF Events\%A_LoopFileName%
        logData := JSON.load(logData)
        msgbox % JSON.dump(logData)
    }
    return
*/