    ...
    if (message.guild) {
        if (message.guild.id=='865904902941048862' && message.content=='!rhino') {
            ...
            async function kekfunc(msg,i) {
                ...
                i++
                if (i<50)
                    msg.edit(postdata).then(msg => setTimeout(kekfunc(kek,i),1000)).catch(err=>console.log(err))
            }
            return
        }
    }
...
async function trading_bot(message,args,command) {
    ...
    if (!status)
        return Promise.reject()
    //------------------
    const func = await trading_bot_orders_update(message,item_id,item_url,item_name,1).catch(err => console.log('Error occured midway of updating orders'))
    return Promise.resolve()
}
