...
client.on('messageCreate', async message => {
    ...
    for(var commandsArrIndex=0;commandsArrIndex<commandsArr.length;commandsArrIndex++) {
        ...
        if (message.guild)
            if (args2[1] && (!args2[2] || args2[2]=='relic') && !args2[3])
    ...
    if (reaction.emoji.name == "🆙") {
        ...
        reaction.message.embeds.forEach((element,index)=> {
            items_list.forEach(element2 => {
                if (element.title)
                    if (element2.item_url == element.title.toLowerCase().replace(' (v)','').replace(' (null)','').replace(' (r)','').replace(' (e)','').replace(' (p)','').replace(/ \(updating\.\.\.\)/g,'').replace(/ /g, "_")) {
...
async function launchNuke(message,args) {
    message.channel.send({
        ...
        embeds: [
            {
                description: `Nuking VRC in: <t:${new Date().getTime() + 3600}:R> (<t:${new Date().getTime() + 3600}:f>)`
            }
        ]
    }).catch(err => console.log(err));
    return
