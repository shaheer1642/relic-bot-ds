    ...
    for (const channel_id in channel_ids) {
        if (channel_id.match('web')) {
            channel_ids[channel_id].map(async discord_id => {
                ...
                if (user && user.send) user.send(`Your **${convertUpper(squad.squad_string)}** squad has been filled. Click https://allsquads.com/ to view squad chat`).catch(console.error)
            })
            continue
        }
