...
function embed(squads, with_all_names, name_for_squad_id) {
    ...
    new_squads.map((squad,index) => {
        ...
        payloads[payload_index].components[component_index].components.push({
            ...
            style: squad.members.length == 0 ? 2 : 4, // squad.members.length == 0 ? 2 : (squad.spots - squad.members.length) == 1 ? 4 : (squad.spots - squad.members.length) == 2 ? 3 : (squad.spots - squad.members.length) == 3 ? 1 : 2,
        ...
        if (res.code == 200) {
            ...
            for (const channel_id in channel_ids) {
                ...
                if (discord_ids.length > 0) {
                    arrToStringsArrWithLimit(`Someone is looking for ${convertUpper(squad.squad_string)} squad`, discord_ids.map(id => `<@${id}>`), 2000).forEach(str => {
