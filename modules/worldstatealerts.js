        ...
        db.query(`SELECT * FROM worldstatealert`).then(res => {
            ...
            if (voidTrader.active) {
                ...
                voidTrader.inventory.forEach((item, index) => {
                    ...
                    db.query(`UPDATE items_list SET vault_status='B',vault_timestamp=${new Date(voidTrader.activation).getTime()} WHERE item_url='${item.item.toLowerCase().replace(/ /g,'_').replace('(intact)','')}'`).catch(err => console.log(err))
