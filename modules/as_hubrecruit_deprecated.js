        ...
        for (const [index,squad] of squadArr.entries()) {
            ...
            if (squad.id == 'sq_leave_all') {
                ...
                components[k][l].components.push({
                    ...
                    style: squad.filled.length == 0 ? 2 : (squad.spots - squad.filled.length) == 1 ? 4 : (squad.spots - squad.filled.length) == 2 ? 3 : (squad.spots - squad.filled.length) == 3 ? 1 : 2, //== 4 ? 2:squad.filled.length == 3 ? 4:squad.filled.length == 2 ? 3:squad.filled.length == 1 ? 1:2,
                    custom_id: squad.id,
                    emoji: squad.emoji
                })
            }
...
function mention_users(joined_user_id,squad_id) {
    ...
    .then(wfhub_squads_data => {
        ...
        .then(wfhub_recruit_members => {
            ...
            if (joined_members.filter(o => o.squad_type == squad_id).length > 1 || joined_members.filter(o => o.squad_type == squad_id).length == 0) return
            var mention_list = []
...
function open_squad(squad) {
    ...
    db.query(`
        ...
        UPDATE as_rb_squads SET members=members${squad.filled.map(discord_id => `-'${discord_id}'`).join('')} WHERE status='active';
