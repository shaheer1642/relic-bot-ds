...
import {
    ...
    FormGroup, FormLabel, FormControlLabel, Checkbox, CircularProgress, Alert, Radio, RadioGroup, IconButton, Grid
} from '@mui/material';
...
class AllSquadsFaqs extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            faqs: [],
                            ...
                            this.state.faqs.map(faq => {
                                return (
                                    <Grid container style={{ border: '5px solid #651fff', borderRadius: '20px', padding: '20px' }} xs={6}>
                                        ...
                                        <Grid item xs={1}>
                                            <Button size='large' disabled={faq.disable_delete} onClick={() => socket.emit('allsquads/faqs/delete', { faq_id: faq.faq_id, user_id: this.props.user?.user_id, }, (res) => this.fetchFaqs())}><Delete fontSize='large' sx={{ color: Colors.red[900] }} /></Button>
                                        </Grid>
                                        <Grid item xs={12}>
...
export default withHooksHOC(AllSquadsFaqs)